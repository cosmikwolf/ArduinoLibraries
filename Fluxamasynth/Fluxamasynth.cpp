...
/*
Fluxamasynth::Fluxamasynth(HardwareSerial * serial) {
    _HardSerial = serial;
    synthInitialized = 0;                                        // Initialization needs to be done
}
*/
...
void Fluxamasynth::filterCutoff(byte channel, byte v) {
    // Bnh 63h 01h 62h 20h 06h vv
    byte command[7] = {0xb0 | (channel & 0x0f), 0x63, 0x01, 0x62, 0x20, 0x06, (v & 0x7f)};
    this->fluxWrite(command, 7);
}

void Fluxamasynth::filterResonance(byte channel, byte v) {
    // Bnh 63h 01h 62h 20h 06h vv
